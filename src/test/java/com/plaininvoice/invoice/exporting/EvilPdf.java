...
final class EvilPdf {
  ...
  private static PageFrame frame() {
    return new PageFrame(1, header(), body(), new FooterToken("Footer"));
  }

  private static HeaderToken header() {
    return new HeaderToken("Invoice", "INV-PDF", LocalDate.of(2026, 1, 1), "Draft");
  }
  ...
  private static PartyToken party(String role) {
    return new PartyToken(role, new DocumentParty("Party", "TAX", "party@example.com"));
  }
  ...
  private static LineToken line() {
    return new LineToken(1, "Line\nvalue", quantity(), percent(), amounts(), new BreakHint("auto", "avoid", "auto"));
  }
  ...
  private static TermsToken terms() {
    return new TermsToken(LocalDate.of(2026, 1, 2), "Net 1");
  }
