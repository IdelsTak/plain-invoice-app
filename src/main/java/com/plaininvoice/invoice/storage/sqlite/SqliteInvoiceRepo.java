...
import com.plaininvoice.invoice.storage.audit.*;
import java.sql.*;
import java.time.*;
import java.util.*;
...
public final class SqliteInvoiceRepo implements InvoiceRepository {
    ...
    private static final AuditOrigin SYSTEM_ORIGIN = new AuditOrigin("system", "sqlite-repo");
    ...
    SqliteInvoiceRepo(Connection connection, InvoiceMapping mapping) {
        this(
          connection,
          mapping,
          new JdbcSqliteRepoPort(connection),
          () -> UUID.randomUUID().toString()
        );
    }

    SqliteInvoiceRepo(Connection connection, InvoiceMapping mapping, SqliteRepoJdbcPort jdbc, Supplier<String> operationIds) {
        ...
        this.operationIds = Objects.requireNonNull(operationIds, "audit operation ids cannot be null");
    ...
    public Optional<StoredInvoice> load(String id) {
        ...
        return head.map(row ->
          new StoredInvoice(row.meta(), mapping.invoice(new InvoiceRows(row, readLines(id), readTaxes(id)))));
    }
    ...
    private AuditTrace trace() {
        return new AuditTrace(operationIds.get(), SYSTEM_ORIGIN);
    }

    private InvoiceAuditEvent audit(InvoiceStoreMeta meta, boolean create, AuditTrace trace) {
        ...
        return new InvoiceAuditEvent(meta.key().id(), meta.key().version(), meta.clock().updatedAt(), kind, trace, detail);
    }

    private void insertConflict(InvoiceStoreMeta meta, StoreConflict conflict, AuditTrace trace) {
        beginConflict();
        try {
    ...
    private void requireDraft(InvoiceStoreKey key) {
        try {
            var stmt = jdbc.prepareStatement("SELECT state FROM invoices WHERE id=? AND version=?");
    ...
    private void beginConflict() {
        try {
            jdbc.execute("BEGIN IMMEDIATE");
        } catch (SQLException ex) {
            throw new IllegalStateException("invoice conflict audit begin failed", ex);
        }
    }
    ...
    private void commitConflict() {
        try {
            jdbc.execute("COMMIT");
        } catch (SQLException ex) {
            throw new IllegalStateException("invoice conflict audit commit failed", ex);
        }
    }
    ...
    private void rollbackConflict() {
        try {
            jdbc.execute("ROLLBACK");
        } catch (SQLException ex) {
            throw new IllegalStateException("invoice conflict audit rollback failed", ex);
        }
    }
    ...
    private void insertAudit(InvoiceAuditEvent event) {
        ...
        """
            ...
            stmt.setString(4, event.trace().operationId());
            stmt.setString(5, event.trace().origin().actor());
            stmt.setString(6, event.trace().origin().source());
            stmt.setString(7, text(event.occurredAt()));
            stmt.setString(8, event.detail());
            stmt.executeUpdate();
        } catch (SQLException ex) {
    ...
    private String kind(InvoiceAuditKind kind) {
        return switch (kind) {
            case InvoiceAuditKind.Created _ ->
                "CREATED";
            case InvoiceAuditKind.Updated _ ->
                "UPDATED";
            case InvoiceAuditKind.Conflict _ ->
                "CONFLICT";
        };
    }
